...
export const CourseModulesTab = ({
  ...
  const localModules = draftModules ?? (course.modules || []);
  const isDirty = draftModules !== null;
  ...
  const addModuleItem = (
    moduleIndex: number,
    type: "video" | "document" | "link" | "text" | "embed",
  ) => {
    const newModules = [...localModules];
  ...
  const removeModuleItem = (moduleIndex: number, itemIndex: number) => {
    setAlertConfig({
      ...
      title: "Delete Item",
      message:
        "Are you sure you want to delete this curriculum item? This action cannot be undone.",
      onConfirm: async () => {
        await new Promise((r) => setTimeout(r, 400));
        const newModules = [...localModules];
        const newItems = [...(newModules[moduleIndex].items || [])];
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 dark:border-slate-700 pb-4">
          ...
          {!isStudent && (
            <button
              ...
              className="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl text-sm font-bold transition-colors flex items-center shrink-0 border border-slate-200 dark:border-slate-700"
            >
              ...
              return (
                ...
                >
                  <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-start gap-4">
                    ...
                    {isCompleted ? (
                      <span className="flex items-center px-3 py-1.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-lg text-sm font-semibold whitespace-nowrap">
                        <CheckCircle className="w-4 h-4 mr-2" /> Completed
                      </span>
                    ) : (
                      <button
                        onClick={() => handleCompleteModule(mod.id)}
                        className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-bold transition-colors whitespace-nowrap shadow-sm"
                      >
                        Mark Complete
                      </button>
                    )}
                    ...
                    <div className="p-4 sm:p-5 border-t border-slate-200 dark:border-slate-700">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {mod.media.map((med) => (
                          <div
                            key={med.id}
                            className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2 shadow-sm"
                          >
                            {med.type === "document" ? (
                              <div className="flex items-center justify-between">
                                ...
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="px-3 py-1.5 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-xs font-bold rounded-lg transition shrink-0 ml-2"
                                >
                                  View
                                </a>
                              </div>
                ...
                {mod.items &&
                  mod.items.map((item, itemIndex) => (
                    <div
                      ...
                      draggable
                      onDragStart={(e) => {
                        e.dataTransfer.setData("text/plain", JSON.stringify({ moduleIndex: index, itemIndex }));
                      ...
                      onDrop={(e) => {
                        ...
                        try {
                          ...
                          if (data.moduleIndex === index && data.itemIndex !== itemIndex) {
                             const newModules = [...localModules];
                             const items = [...(newModules[index].items || [])];
                             const [draggedItem] = items.splice(data.itemIndex, 1);
                             items.splice(itemIndex, 0, draggedItem);
                             newModules[index] = { ...newModules[index], items };
                             setDraftModules(newModules);
                          }
                        } catch (err) {
                    ...
                    >
                      ...
                      <div className="p-3 sm:px-4 flex justify-between items-center bg-slate-50 dark:bg-slate-800/80 border-b border-slate-100 dark:border-slate-700/50">
                        <div className="flex items-center gap-3 flex-1">
                          <div className="cursor-grab active:cursor-grabbing text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 p-1 shrink-0 -ml-2">
                        ...
                        <button
                          ...
                          className="text-slate-400 hover:text-red-500 ml-3 transition-colors p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 shrink-0"
                          title="Delete Item"
                        >
                            ...
                            {item.type === "video" && (
                              <div className="flex items-center gap-3">
                                ...
                                <input
                                  ...
                                  className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-white"
                                  placeholder="Paste video URL (e.g. YouTube link) or upload below..."
                                />
                              </div>
                            )}
                            <div className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-8 text-center hover:bg-slate-50 dark:hover:bg-slate-900/50 transition duration-200">
                              ...
                              {item.url && (
                                <div className="mt-4 inline-flex items-center px-4 py-2 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-xl text-sm font-bold border border-emerald-200 dark:border-emerald-800/50">
                                  <CheckCircle2 className="w-4 h-4 mr-2" /> Resource Added
                                </div>
                              )}
                              {!item.url && (
