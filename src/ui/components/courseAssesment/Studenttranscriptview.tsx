...
import { Award, Paperclip, X, CheckCircle2, Circle, Info, Accessibility, Volume2 } from "lucide-react";
...
export function StudentTranscriptView({
  ...
  const { openAccessibilityModal, examTimeMultiplier, setTtsEnabled, ttsEnabled } = useAccessibility();
                ...
                {!sub && (
                  <div className="mt-4 pt-4 border-t border-slate-800">
                    ...
                    activeProctoringId !== ass.id ? (
                      ...
                      <div className="space-y-4">
                        {/* Assessment Accessibility Helper Toolbar */}
                        <div className="flex flex-wrap items-center justify-between gap-2 p-3 bg-indigo-50/70 dark:bg-indigo-950/30 border border-indigo-200/80 dark:border-indigo-900/50 rounded-xl text-xs">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            >
                              ...
                              <span>{ttsEnabled ? "TTS On" : "TTS Reader"}</span>
                            </button>
                            <button
                              type="button"
                              onClick={() => openAccessibilityModal()}
                              className="px-2.5 py-1 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-[11px] font-bold transition"
                            >
                              Engine Settings
                            </button>
                          </div>
                        ...
                        {(ass.type === "exam" || ass.type === "test") &&
                          activeProctoringId === ass.id && (
                            <ProctoringSession
                              ...
                              durationMinutes={ass.durationMinutes || 60}
                              onComplete={() => {}}
                            />
                          )}
